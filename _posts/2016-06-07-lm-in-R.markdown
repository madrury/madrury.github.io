---
layout: post
title:  "How R Fits a Linear Model "
...
A `call` object is a small wonder, it can be indexed into and manipulated much like other R objects.  The first index always gives the name of the function that was called (not as a string, as a `symbol` object), the others give the arguments passed into the call
...
replaces `lm` in the call object with `model.frame` (the `quote` creates a symbol out of the name).  Similarly, the lines
...
Attached to this is a [terms](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/terms.object.html) attribute, which contains metadata needed to identify which column represents the response, and which the predictors (the encoding of the information is pretty obscure, so I wont bother to unwind it here).  Using the data frame and the terms data, we can construct the final design matrix
