...
A Householder reflection is a linear transformation that preforms a reflection about a hyperplane (a hyperplane is a subspace of dimension one less than the enclosing space).  A hyperplane can be uniquely described by its unit normal vector $$v$$, in which case the corresponding Householder reflection is the mapping
...
Again, the fortran code is convoluted enough that it's not worth it to untangle its knots, but let's take a stock of where we are.  We've passed down through many layers R, then C, and finally fortran code from the early days of computing.  At the very bottom, we finally recieve the solution to the linear regression problem we constructed in R at the very top of the stack.

Wrap Up
-------

If you've read this far, I hope that you've found this journey through one of the most basic and fundamental R functions enjoyable.  Many expositions of linear regression focus on the mathematics, and stop after deriving the equation
